...
export default function ShoppingCardForm({ isOpen, setIsOpen }) {
  ...
  useEffect(() => {
    ...
    basket.forEach((product) => {
      if (!initialQuantities[product.id]) {
        ...
        initialQuantities[product.id].quantity += 1;
      }
    });
  ...
  return (
    <>
          ...
          <div className="bg-white w-4/5 md:w-2/5 lg:w-1/3 h-full p-4 lg:p-8 transform transition-transform duration-500 flex flex-col justify-between">
            ...
            <div className="flex flex-col w-full">
              ...
              <div className="flex justify-between items-center w-full">
                ...
                <button onClick={handleCheckout} className="text-right text-2xl bg-black text-white p-2 rounded">CHECK OUT</button>
              </div>
            </div>
      ...
      <CheckOutModal onContinueShopping={onContinueShopping}
        onSignIn={onSignIn} />
    </>
  );
}
